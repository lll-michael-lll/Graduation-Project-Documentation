...
\begin{sectionintro}{9}{Technical Background}{
  \begin{itemize}[leftmargin=1.5em]
    \item Go programming language and systems programming
    \item Database wire protocols (MySQL, MSSQL)
    \item Zero Trust security architecture principles
    \item Authentication mechanisms (JWT, OAuth, MFA)
    \item Frontend technologies (React, TypeScript)
    \item DevOps and deployment considerations
  \end{itemize}
}
\lettrine[lines=3, lhang=0.1, loversize=0.15]{\color{primaryBlue}U}{nderstanding the technical foundations underlying} \textbf{zGate} is essential for appreciating the architectural decisions and implementation approaches. This chapter provides comprehensive coverage of all technologies, protocols, and concepts that form the building blocks of the system. From low-level wire protocol handling to modern frontend frameworks, each technology choice is explained with academic rigor.
\end{sectionintro}
