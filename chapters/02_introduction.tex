...
\begin{sectionintro}{2}{Introduction \& Problem Definition}{
  \begin{itemize}[leftmargin=1.5em]
    \item Database access vulnerabilities and credential exposure
    \item Traditional security solution limitations
    \item Zero Trust principles for database security
    \item Identity-based access control implementation
  \end{itemize}
}
In modern enterprises, databases store critical assets yet traditional access management creates fundamental security vulnerabilities. This chapter examines how static credentials, shared accounts, and legacy authentication patterns expose organizations to breaches and compliance failures. We explore the security imperative driving Zero Trust database access control and establish the foundational problems that zGate addresses.
\end{sectionintro}
...
\textbf{Gateway Server:} Implements protocol handlers for PostgreSQL, MySQL and MSSQL, intercepting all database traffic at the wire protocol level. A six-phase \texttt{Dispatcher} (PeekRouting \textrightarrow{} StartSession \textrightarrow{} Bind \textrightarrow{} Backend Dial \textrightarrow{} Handshake \textrightarrow{} Command Loop) orchestrates every connection. An eight-stage interceptor pipeline then applies rate limiting, T-SQL safety analysis, column masking, AI-assisted PII redaction, row-level filtering, and ML anomaly detection to every query.

\textbf{Command-Line Interface:} Provides developers with a Bubble Tea TUI (and a headless mode for CI/CD). Users authenticate via OIDC Device Authorization Grant against Keycloak---no password is ever transmitted to the CLI. Short-lived mTLS certificates and a local loopback tunnel allow standard database clients to connect without modification.

\textbf{Web Administration Dashboard:} Built with Next.js 15, React 19, and iron-session, the dashboard enables security teams to define RBAC policies, configure database connections, manage compliance frameworks, and monitor active sessions. Vault AppRole secrets bootstrap keeps all sensitive configuration out of environment files.

\textbf{AI Engine, T-SQL Parser, and Deploy Stack:} Three supporting components provide AI-assisted column masking (hot-swappable LLM providers via gRPC), structural T-SQL AST analysis (40+ SQL features via gRPC sidecar), and one-command deployment via Docker Compose with full observability (Jaeger, Prometheus, Loki, Grafana).
