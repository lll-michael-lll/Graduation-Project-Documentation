...
\begin{sectionintro}{4}{Requirements Engineering}{
  \begin{itemize}[leftmargin=1.5em]
    \item Functional requirements across all six platform components
    \item Non-functional requirements and measurable quality targets
    \item System actors: human users, platform services, and AI/analysis sidecars
    \item Use case catalog: authentication, connection, masking, anomaly, and policy flows
    \item Second-semester requirement additions and scope updates
  \end{itemize}
}
...
\begin{itemize}
    \item OIDC Device Authorization Grant via Keycloak for CLI users (no password transmitted to CLI)
    \item PKCE/OIDC browser flow for WebUI administrators
    \item ALTCHA SHA-256 Proof-of-Work verification on all direct login endpoints
    \item JWT-based session tokens (HS256): access token (5-minute default, configurable via \texttt{ZGATE\_ACCESS\_TOKEN\_DURATION}), refresh token (1-hour default, configurable via \texttt{ZGATE\_REFRESH\_TOKEN\_DURATION})
    \item Per-token revocation via JTI (JWT ID) store; per-session revocation via refresh token family ID
    \item Device fingerprint binding (SHA-256 of IP + User-Agent, 128-bit default) in every JWT to reject stolen-token replay
    \item Mandatory TOTP two-factor authentication for all admin accounts
    \item Role-based access control (RBAC) with four admin tiers: \texttt{super}, \texttt{db\_admin}, \texttt{policy\_admin}, \texttt{auditor}
    \item Custom user permissions that combine with role-based permissions
\end{itemize}

\subsection{FR-2: Database Connection Management}
\begin{itemize}
    \item Multi-protocol proxy support: MySQL (port 9000), PostgreSQL (port 9001), Microsoft SQL Server/TDS (port 9002)
    \item Two credential modes: \textit{ephemeral} (per-session temporary database users with auto-cleanup) and \textit{shared} (pooled shared accounts with concurrency limits)
    \item Ticket mode: short-lived signed connection ticket; mTLS strict mode as opt-in alternative
    \item Automatic resource cleanup on session close within 30 seconds of disconnect
    \item Per-database health checks at configurable intervals
\end{itemize}

\subsection{FR-3: Policy Engine}
\begin{itemize}
    \item Real-time access control validation per connection and per query
    \item Safety templates with four action types: \texttt{allow}, \texttt{deny}, \texttt{warn} (requires admin review), and two distinct \texttt{mask} mechanisms:
    \begin{itemize}
        \item \textbf{AI Column Masking:} Server calls AI Engine \texttt{/classify-columns}; LLM maps column names and sample values to masking strategies. Result is cached and enforced by the AIMasking interceptor stage.
        \item \textbf{Rule-based Row Masking:} Configured static masking policies applied by the RowMasking interceptor stage; no AI involved.
    \end{itemize}
    \item Multi-level permissions (read, write, admin) enforced at the wire level
    \item Warn flow: queries matching warn rules are held pending admin approval
    \item 6 pre-seeded regulatory compliance frameworks: GDPR, HIPAA, PCI-DSS, SOX, ISO~27001, NIST
\end{itemize}

\subsection{FR-4: Command Line Interface}
\begin{itemize}
    \item Auto access token refresh (access \& refresh tokens)
    \item Login methods: username/password (local) and OAuth2/OIDC (external IdP)
    \item Database listing and connection
    \item Session status monitoring
    \item mTLS certificate lifecycle management (CSR-first issuance via Server PKI, private key never leaves client, auto-renewal at 25\% lifetime or CA rotation)
    \item BubbleTea TUI interactive dashboard (Model/Update/View stateful terminal UI)
    \item Headless/CI mode (subcommands operable without TTY for pipeline automation)
    \item Local tunnel bridge (loopback listener — Unix socket on Linux/macOS, TCP on Windows — that bridges plaintext database client traffic to TLS 1.3 mTLS proxy connection)
\end{itemize}
...
\begin{itemize}
    ...
    \item T-SQL Parser gRPC dependency: for MSSQL, every query passes through the T-SQL Parser sidecar via gRPC before policy evaluation, affecting latency and failure handling
    \item Wire-level interception with protocol-specific handshake injection for ephemeral credentials
    \item Six-phase Dispatcher architecture: PeekRouting \textrightarrow{} StartSession \textrightarrow{} Bind \textrightarrow{} Backend Dial \textrightarrow{} Handshake \textrightarrow{} Command Loop
\end{itemize}

\subsection{FR-7: Audit \& Logging}
\begin{itemize}
    \item Structured OTLP telemetry export to OTel Collector (Jaeger/Prometheus/Loki/Grafana)
    \item Per-query NDJSON audit log written to \texttt{ZGATE\_QUERY\_LOG\_DIR} with rotation
    \item All authentication events logged with user context, IP, and outcome
    \item Sensitive values (passwords, credentials) never logged; query text truncated in sidecar logs
\end{itemize}

\subsection{FR-8: AI/ML Engine}
\begin{itemize}
    \item LLM-based column sensitivity classification (\texttt{/classify-columns} endpoint)
    \item Behavioral anomaly detection via ML ensemble (\texttt{/anomaly/detect} endpoint)
    \item Pluggable LLM provider selection without redeploy
    \item Fail-open contract for anomaly (errors never block traffic) and fail-closed contract for masking (timeouts withhold data rather than leak it)
    \item Admin FP/TP feedback endpoint (\texttt{/anomaly/feedback}) that feeds online model retraining
    \item Server-managed LLM config pulled from \texttt{/api/internal/llm-config} at boot and refreshed periodically
    \item Vault AppRole for secret access
\end{itemize}

\subsection{FR-9: T-SQL Parser Sidecar}
\begin{itemize}
    \item Structural T-SQL AST analysis via gRPC
    \item Extraction of 40+ query features (query type, tables, columns, schemas, WHERE clauses, JOINs, subqueries, CTEs, locking hints)
    \item Threat detection for SQLi bypass patterns (tautologies, tautological JOINs, dynamic SQL execution)
    \item Data exfiltration vector detection (OPENROWSET, OPENQUERY, BACKUP/RESTORE, system schema access)
    \item Query fingerprinting (normalize literals to placeholders, SHA-256 fingerprint)
    \item Column masking resolution (traces each SELECT output column back to its source table and column for masking rule application)
    \item gRPC communication over Unix socket or TCP with the Server
\end{itemize}
...
\textbf{Key Security Parameters:}
\begin{itemize}
    \item Access token expiry: 5 minutes (\texttt{ZGATE\_ACCESS\_TOKEN\_DURATION})
    \item Refresh token expiry: 1 hour (\texttt{ZGATE\_REFRESH\_TOKEN\_DURATION})
    \item Proxy ticket TTL: 60 seconds (\texttt{PROXY\_TICKET\_TTL\_SECONDS})
    \item Password hashing: bcrypt, work factor 12 (\texttt{ZGATE\_BCRYPT\_COST})
    \item Session isolation with temporary users; auto-cleanup within 30 seconds
\end{itemize}

\subsection{NFR-2: Performance and Latency}
\begin{itemize}
    \item Proxy added latency budget per query strictly monitored
    \item T-SQL Parser gRPC round-trip target must be synchronous and optimized, as it is on the hot path for every MSSQL query
    \item AI Engine inference timeout thresholds strictly enforced (fail-closed timeouts are documented as requirements)
    \item Connection pool sizing targets implemented to maintain throughput under concurrent session load
\end{itemize}
...
\begin{itemize}
    ...
    \item Configuration persisted in MySQL (not YAML)
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/actor-diagram.png}
    \caption{System actors and components: End Users (via CLI), System and Super Administrators (via WebUI), and the six platform components alongside Keycloak and backend databases.}
...
\begin{itemize}
    \item \textbf{Description:} Target database systems (MSSQL, MySQL, PostgreSQL) that users need to access
    \item \textbf{Responsibilities:}
    \begin{itemize}
...
\subsubsection{zGate AI Engine}
\begin{itemize}
    \item \textbf{Description:} Non-interactive ML inference sidecar participating in security decisions
    \item \textbf{Responsibilities:}
    \begin{itemize}
        \item Classify columns for masking strategy on first table access
        \item Score every query for behavioral anomaly
        \item Accept admin feedback for model retraining
        \item Pull LLM configuration from Server at boot
    \end{itemize}
\end{itemize}

\subsubsection{zGate T-SQL Parser}
\begin{itemize}
    \item \textbf{Description:} Non-interactive sidecar invoked on every MSSQL query via gRPC before policy evaluation
    \item \textbf{Responsibilities:}
    \begin{itemize}
        \item Parse incoming T-SQL into AST and extract structural features
        \item Flag threat patterns and generate query fingerprint
        \item Resolve masking column origins for SELECT output
    \end{itemize}
\end{itemize}
...
\textbf{Main Flow:}
\begin{enumerate}
    \item User executes \texttt{zgate connect <db\_name>}
...
\begin{enumerate}
    ...
    \item Server persists configuration to the MySQL store
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/enduser-use-cases.png}
    \caption{End user use cases via zGate-CLI: OIDC Device Flow authentication (UC-1), database listing with RBAC filtering (UC-2), mTLS loopback tunnel connection with ephemeral credentials (UC-3), automatic token refresh (UC-10), and JTI logout (UC-11).}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/administrator-use-cases.png}
    \caption{Administrator use cases via zGate-WebUI: database configuration (UC-5), user/role management (UC-6), policy template definition (UC-7), session monitoring (UC-8), audit log review (UC-9), shared account pools (UC-12), compliance templates, warn-queue review, and AI/LLM provider configuration.}
    \label{fig:administrator-use-cases}
\end{figure}
\subsection{Complete System Use Case Diagrams}
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/complete-system-use-case.png}
    \caption{Complete system use case diagram: all actors and six platform components, from CLI and WebUI flows through zGate-Server to the T-SQL Parser, AI Engine sidecars, and backend databases.}
    \label{fig:complete-system-use-case}
\end{figure}
\subsection{System Components Interaction}
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/system-component-interaction.png}
    \caption{System component interaction across all six platform components: Client Layer to Server API through Auth, Policy Engine, six-phase Dispatcher, and eight-stage Interceptor Pipeline, with gRPC sidecars, MySQL Store, Vault, and OTel export.}
...
\noindent\textbf{Acceptance Criteria:}
\begin{itemize}
    \item Command \texttt{zgate connect <db\_name>} initiates connection
...
\begin{itemize}
    ...
    \item Configuration is persisted to the MySQL store
