...
\textbf{2. Safety Interceptor}

\textit{Purpose:} Prevents dangerous database operations based on configurable rules.
...
\textbf{3. Masking Interceptor}

\textit{Purpose:} Dynamically masks sensitive data in query results based on column names.
...
\noindent\textbf{Composite Interceptor:} The CompositeInterceptor orchestrates the execution of all configured interceptors in sequence, maintaining the chain of responsibility pattern.
...
The gateway supports multiple authentication schemes to suit different deployment and operational needs:
\begin{itemize}[leftmargin=*]
    \item \textbf{Username / Password (Local):} Traditional credential-based login where passwords are verified (bcrypt) and a JWT pair (access + refresh) is issued for subsequent API access.
    \item \textbf{OAuth2 / OIDC (External IdP):} Integration point for enterprise identity providers to enable Single Sign-On (SSO) and federation; zGate consumes identity tokens and issues local JWTs as needed.
    \item \textbf{Mutual TLS (mTLS) / Service Certificates:} Certificate-based authentication for automated clients and service accounts where client certificates are validated during the TLS handshake and mapped to principal identities.
\end{itemize}
...
\textit{Reliability Goal:} 99.9\% uptime for gateway layer, with automatic recovery from transient failures and comprehensive logging for post-mortem analysis.
